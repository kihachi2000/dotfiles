local vsnip_tab = function()
    if vim.fn["vsnip#available"](1) == 1 then
        local key = vim.api.nvim_replace_termcodes("<Plug>(vsnip-jump-next)", true, true, true)
...
local vsnip_shift_tab = function()
    if vim.fn["vsnip#available"](-1) == 1 then
        local key = vim.api.nvim_replace_termcodes("<Plug>(vsnip-jump-prev)", true, true, true)
